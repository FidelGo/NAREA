...
processMarkdown <- function(py, px, r, d, I, counter) {
  ...
  template_path <- "/Users/SHSU/R_work/NAREA/SecondStep/PerfectSubs_X_template.Rmd"
  ...
  output_file_name <- sprintf("PerfSub_numeric_%d.Rmd", counter)
...
exm <- cbind(sapply(1:totfile, function(i) paste0("PerfSub_numeric_", i, ".Rmd")))

# Before calling exams2blackboard, ensure exm contains the correct file paths
exams2blackboard(exm, name="perfect_substitutes_x")
