...
generateRandomDataFrame <- function(n = 1000) {
  ...
  df <- data.frame(
    ...
    b = sample(200:1000, n, replace = TRUE),
...
set.seed(123)
randomDataFrame <- generateRandomDataFrame(100)

# Define the processMarkdown function
processMarkdown <- function(a, b, c, f, counter) {
  # Read the markdown template
  template_path <- "/Users/SHSU/R_work/NAREA/SecondStep/SandD_Equilibrium_P_Food.Rmd"
...
#  processed_text <- gsub("\\{y\\}", y_random, processed_text)  # Substitute {y} with the random element
  ...
  output_file_name <- sprintf("SandD_price_food_numeric_%d.Rmd", counter)
...
exm <- cbind(sapply(1:totfile, function(i) paste0("SandD_price_food_numeric_", i, ".Rmd")))

# Before calling exams2blackboard, ensure exm contains the correct file paths
exams2blackboard(exm, name="SandD_price_food")
